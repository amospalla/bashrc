...
_bashrc_show_help(){
	...
	color blue; printf "folder-exists"; color; echo "check if a list of folders exist."
	color blue; printf "bashrc-update"; color; echo ": manage files updated by bashrc."
	color blue; printf "section"; color; echo ": print start/end position of a part of a file."
	color blue; printf "muttrc"; color; echo ": manager and execute mutt profiles."
...
_source_utilities(){
	...
	box(){
		...
		args1='[-d|--delimiter {delimiter}] [-s|--size {bordersize:integer}] [-b|--bordercolor {bcolor}] [-t|--textcolor {tcolor}] {text}'
		arguments_description=('box' 'Print a text surrounded by a box.')
		arguments_parameters=( '[-d|--delimiter {delimiter}]: character used for printing box.'
		                       '[-s|--size {bordersize}]: border width.'
		                       '[-b|--bordercolor {bcolor}]: color used for border.'
		                       '[-t|--textcolor {tcolor}]: color used for text.'
							   '{text}: text to print.')
		local -A arguments=()
		argparse "$@" && shift ${arguments_shift}
		declare -a text
		...
		for (( i=0; i<${arguments[bordersize]:-2}; i++ )); do
	...
	filewatch(){
		arguments_list=(args1)
		args1='[-c|--continuous [-i|--interval {interval:time}]] {file} [command...]'
		arguments_description=('filewatch' 'Notify when a file has been modified, optionally execute in loop and/or execute a command.')
		arguments_parameters=( '[-c|--continuous [-i|--interval {interval}]]: run continuously with a default interval of 0.5 seconds.'
		                       '{file}: file to monitor.'
		                       '[command...]: execute a command on change.' )
		local -A arguments=()
		argparse "$@" && shift ${arguments_shift}
		...
		date_new=${date_old}
		while "true"; do
			while [[ ${date_new} -eq ${date_old} ]]; do
				date_new="$(stat -c %Y "${arguments[file]}")"
				sleep ${arguments[interval]:-0.5}
			done
			[[ ${#} -gt 0 ]] && "${@}"
	...
	section(){
		arguments_list=(args1)
		args1='{startmark} {endmark} {file}'
		arguments_description=('section' 'Outputs where a section resides inside a file (including marks).')
		arguments_parameters=('{startmark}: text that marks the start of a section.'
		                      '{endmark}: text that marks the end of a section.'
		                      '{file}: programs to check for.' )
		local -A arguments=()
		argparse "$@" && shift ${arguments_shift}
		file-readable -m "${arguments[file]}" || exit 1
	...
	folder-exists(){
		arguments_list=(args1)
		args1='[-m|--message] [-i|--invert] {folders...}'
		arguments_description=('folder-exists' 'Check if a folder exists.')
		arguments_parameters=('[-m|--message]: print a message if logic fails.'
		                      '[-i|--invert]: invert the logic of the check.'
		                      '{folders...}: folders to check for.' )
		local -A arguments=()
		argparse "$@" && shift ${arguments_shift}
		while [[ $# -gt 0 ]]; do
			if [[ -d "${1}" && ${arguments[-i]:-0} -eq 1 ]]; then
				[[ ${arguments[-m]:-0} -eq 1 ]] && echo "Error: folder '$1' exists."; exit 1
			fi
			if [[ ! -d "${1}" && ${arguments[-i]:-0} -eq 0 ]]; then
				[[ ${arguments[-m]:-0} -eq 1 ]] && echo "Error: '$1' does not exist or is not a folder."; exit 1
			fi
			shift
		done
	}
	...
	lvmthinsnapshots(){
		...
		# Check if thinpool data and metadata are below threshold
	...
	_history_compact(){
		...
		for (( i=0; i<${#days[@]}; i++ )); do
			for file in "${path}/${days[$i]}"*; do
				...
				# Skip if pid proc exists, uid does match and process is bash
				[[ -f /proc/${pid}/comm && $(</proc/${pid}/status) =~ Uid:[[:blank:]]*${EUID}[[:blank:]] && $(tr -d '\0' < /proc/${pid}/cmdline) =~ -?bash ]] && continue 2
			done
